...
import bgu.spl.mics.impl.RoundRobinList;
...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<String,Callback> openingCallbacks;
    private ConcurrentHashMap<String,Callback> onCompleteCallbacks;
    ...
    public MicroService(String name) {
        this.name = name;
        openingCallbacks = new ConcurrentHashMap<String,Callback>();
        onCompleteCallbacks = new ConcurrentHashMap<String,Callback>();
    }
    ...
    protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
    	...
    	openingCallbacks.put(type.getName(), callback);
        messageBus.subscribeRequest(type, this);
        ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" subscribed to request "+type.getName());
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        ...
        ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" subscribed to broadcast "+type.getName());
    }
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
    	MessageBus messageBus = MessageBusImpl.getInstance(); 
    	onCompleteCallbacks.put(r.getClass().getName(), onComplete);
    	ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" sent request "+r.getClass().getName());
        return messageBus.sendRequest(r, this);
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
    	MessageBus messageBus = MessageBusImpl.getInstance(); 
    	ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" sent broadcast "+b.getClass().getName());
    ...
    protected final <T> void complete(Request<T> r, T result) {
    	MessageBus messageBus = MessageBusImpl.getInstance(); 
    	ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+this.getName()+" sent 'complete' to MB");
    ...
    protected final void terminate() {
        ...
        ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" terminated");
    }
    ...
    public final void run() {
    	MessageBus messageBus = MessageBusImpl.getInstance(); 
    	ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+name+" is trying to register");
        ...
        while (!terminated) {
        	try{
	        	...
	        	if(msg.getClass() == RequestCompleted.class){
	        		onCompleteCallbacks.get(((RequestCompleted) msg).getCompletedRequest().getClass().getName()).call(((RequestCompleted) msg).getResult());
	        	}
	        	else {
	        		openingCallbacks.get(msg.getClass().getName()).call(msg);
	   
	        	}
        	} catch (InterruptedException ex){};
        	
        }
        ShoeStoreRunner.LOGGER.log(Level.INFO, "service "+this.getName()+"is trying to unregister");
